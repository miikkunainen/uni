...
(function(){
  ...
  // ---------- MUUTOSKERROS ----------
  /* Perusdata yllä on muuttumaton pohja. Henkilökunnan tekemät
     lisäykset, korjaukset ja poistot tallentuvat erilliseen
     muutoskerrokseen ja yhdistetään pohjaan lukuhetkellä.

     Poisto on aina pehmeä: tietue säilyy ja saa merkinnän
     deleted = { by, at, reason }. Opiskelija ei näe poistettua;
     henkilökunta näkee sen merkittynä ja voi palauttaa sen. */

  var OV_KEY="paja_muutokset_v1";
  var STAFF_KEY="paja_henkilokunta_v1";

  function ov(){
    try{ var s=localStorage.getItem(OV_KEY); return s?JSON.parse(s):{machines:{},tools:{},log:[]}; }
    catch(e){ return {machines:{},tools:{},log:[]}; }
  }
  function saveOv(o){
    try{ localStorage.setItem(OV_KEY,JSON.stringify(o)); return true; }catch(e){ return false; }
  }
  ...
  function setStaff(on){
    try{ if(on) localStorage.setItem(STAFF_KEY,"1"); else localStorage.removeItem(STAFF_KEY); }catch(e){}
  }

  /* Yhdistä pohja + muutokset. includeDeleted vain henkilökunnalle. */
  function merge(baseArr, kind, opts){
    opts=opts||{};
    var o=ov(), edits=o[kind]||{};
    var out=[], seen={}, i;
    for(i=0;i<baseArr.length;i++){
      var b=baseArr[i], e=edits[b.id];
      var rec;
      if(e){
        rec={}; var k;
        for(k in b){ if(b.hasOwnProperty(k)) rec[k]=b[k]; }
        for(k in e){ if(e.hasOwnProperty(k)) rec[k]=e[k]; }
      } else { rec=b; }
      seen[b.id]=1;
      out.push(rec);
    }
    // henkilökunnan lisäämät uudet
    var id;
    for(id in edits){ if(edits.hasOwnProperty(id) && !seen[id]){ out.push(edits[id]); } }
    // suodata poistetut, ellei erikseen pyydetty
    if(!opts.includeDeleted){ out=out.filter(function(x){ return !x.deleted; }); }
    // kampussuodatus
    if(opts.campus!==false){
      var c=opts.campus||getCampus();
      out=out.filter(function(x){ return !x.campus || x.campus===c; });
    }
    return out;
  }

  function upsert(kind, rec, by){
    if(!rec || !rec.id) return false;
    var o=ov();
    o[kind]=o[kind]||{};
    var isNew=!o[kind][rec.id];
    o[kind][rec.id]=rec;
    o.log=o.log||[];
    o.log.push({ ts:nowISO(), kind:kind, id:rec.id, name:rec.name||"",
                 action:isNew?"lisatty":"muokattu", by:by||"" });
    return saveOv(o);
  }
  function softDelete(kind, id, by, reason){
    var o=ov(); o[kind]=o[kind]||{};
    var base=(kind==="machines"?MACHINES:TOOLS).filter(function(x){ return x.id===id; });
    var cur=o[kind][id];
    if(!cur){
      if(base.length===0) return false;
      cur={}; var k, b=base[0];
      for(k in b){ if(b.hasOwnProperty(k)) cur[k]=b[k]; }
    }
    cur.deleted={ by:by||"", at:nowISO(), reason:reason||"" };
    o[kind][id]=cur;
    o.log=o.log||[];
    o.log.push({ ts:nowISO(), kind:kind, id:id, name:cur.name||"", action:"poistettu", by:by||"", reason:reason||"" });
    return saveOv(o);
  }
  function restore(kind, id, by){
    var o=ov(); o[kind]=o[kind]||{};
    var cur=o[kind][id];
    if(!cur) return false;
    delete cur.deleted;
    o[kind][id]=cur;
    o.log=o.log||[];
    o.log.push({ ts:nowISO(), kind:kind, id:id, name:cur.name||"", action:"palautettu", by:by||"" });
    return saveOv(o);
  }
  ...
  window.PAJA={
    ...
    rooms:function(c){ c=c||getCampus(); return ROOMS.filter(function(r){ return !r.campus||r.campus===c; }); },
    roomNames:function(c){ return window.PAJA.rooms(c).map(function(r){ return r.name; }); },
    roomCode:function(name){ var a=ROOMS.filter(function(r){ return r.name===name; }); return a.length?a[0].code:""; },

    // koneet: opiskelijalle näkyvät (ei poistettuja)
    machines:function(c){ return merge(MACHINES,"machines",{campus:c}); },
    // henkilökunnalle: myös poistetut
    allMachines:function(c){ return merge(MACHINES,"machines",{campus:c, includeDeleted:true}); },
    machine:function(id){ var a=merge(MACHINES,"machines",{campus:false,includeDeleted:true})
      .filter(function(m){ return m.id===id; }); return a.length?a[0]:null; },
    machinesInRoom:function(room,c){ return merge(MACHINES,"machines",{campus:c})
      .filter(function(m){ return m.room===room; }); },
    saveMachine:function(rec,by){ return upsert("machines",rec,by); },
    deleteMachine:function(id,by,reason){ return softDelete("machines",id,by,reason); },
    restoreMachine:function(id,by){ return restore("machines",id,by); },

    tools:function(c){ return merge(TOOLS,"tools",{campus:c}); },
    allTools:function(c){ return merge(TOOLS,"tools",{campus:c, includeDeleted:true}); },
    tool:function(id){ var a=merge(TOOLS,"tools",{campus:false,includeDeleted:true})
      .filter(function(x){ return x.id===id; }); return a.length?a[0]:null; },
    saveTool:function(rec,by){ return upsert("tools",rec,by); },
    deleteTool:function(id,by,reason){ return softDelete("tools",id,by,reason); },
    restoreTool:function(id,by){ return restore("tools",id,by); },
    ...
    // muutoskerros
    changes:function(){ return ov(); },
    changeLog:function(){ var o=ov(); return (o.log||[]).slice(); },
    changeCount:function(){ var o=ov();
      return Object.keys(o.machines||{}).length + Object.keys(o.tools||{}).length; },
    deletedCount:function(){
      var d=merge(MACHINES,"machines",{campus:false,includeDeleted:true}).filter(function(x){ return x.deleted; }).length;
      d+=merge(TOOLS,"tools",{campus:false,includeDeleted:true}).filter(function(x){ return x.deleted; }).length;
      return d; },
    exportChanges:function(){ return JSON.stringify(ov(),null,2); },
    importChanges:function(json){
      try{ var o=JSON.parse(json);
        if(!o || typeof o!=="object") return false;
        o.machines=o.machines||{}; o.tools=o.tools||{}; o.log=o.log||[];
        return saveOv(o);
      }catch(e){ return false; } },
    clearChanges:function(){ return saveOv({machines:{},tools:{},log:[]}); },

    newId:function(prefix){ return (prefix||"x_")+Math.random().toString(36).slice(2,8); },
